...
//            serializer.Expect(x => x.Serialize(objectToSave)).Return(fileBytes);

//            var tempFile = Path.GetTempFileName();

//            FileManager.SaveToFile(objectToSave, tempFile, serializer);
...
//            var serializer = new Mock<IRuntimeSerializer>();
//            serializer.Expect(x => x.Deserialize(fileBytes)).Return(objectToSave);

//            var tempFile = Path.GetTempFileName();
//            File.WriteAllBytes(tempFile, fileBytes);

//            var restored = FileManager.ReadFromFile<string>(tempFile, serializer);
...
//            var status = FileManager.VerifyHash(inexistentFile, Arg<string>.Is.Anything, Arg<IHashProvider>.Is.Anything, false);
...
//            provider.Expect(x => x.HashFile(_testFile)).Return(calcualtedHash);

//            var status = FileManager.VerifyHash(_testFile, originalHash, provider, false);
...
//            const string originalHash = "1234567890abcdefgh";
//            var provider = new Mock<IHashProvider>();
//            provider.Expect(x => x.HashFile(_testFile)).Throw(new AccessViolationException());
...
//        {
//            var provider = new Mock<IHashProvider>();
//            _testHashFile = _testFile + "." + provider.Algorythm;
//            const string originalHash = "1234567890abcdefgh";
//            const string calcualtedHash = "1234567890abcdefgh";
//            provider.Expect(x => x.HashFile(_testFile)).Return(calcualtedHash);

//            var status = FileManager.VerifyHash(_testFile, originalHash, provider, true);
...
//            var provider = new Mock<IHashProvider>();
//            const string originalHash = "1234567890abcdefgh";
//            _testHashFile = CreateHashFile(_testFile, provider, originalHash);

//            var status = FileManager.VerifyHash(_testFile, originalHash, provider, true);

//            status.Should().Be.EqualTo(FileStatus.Present);
//            provider.AssertWasNotCalled(x => x.HashFile(Arg<string>.Is.Anything));
...
//            var provider = new Mock<IHashProvider>();
//            provider.Expect(x => x.HashFile(_testFile)).Return(originalHash);
//            // Creates an old file hash 
//            _testHashFile = CreateHashFile(_testFile, provider, "-----old-hash------");
...
//            FileManager.CleanHash(_testFile, provider);
